...
<body>
	...
	<div class="panel col-sm-4" align="center"
		style="margin-left: 250px; margin-top: -20px;">
		<div class="panel-heading ">
			<h3 class="panel-title "></h3>
		</div>
		<div class="panel-body">
			<form id="regForm" action="updatestud" method="post">
				<div class="form-group">
					<label for="id">Enrollment Number</label> <input
						class="form-control" value="${olduser.id}" disabled /> <input
						type="hidden" name="id" id="id" class="form-control"
						value="${olduser.id}" />
				</div>
				<div class="form-group">
					<label for="name">Name</label> <input name="name" id="name"
						class="form-control" value="${olduser.name}" required="true" />
					<div id="namem" style="color: red;"></div>
				</div>
				<div class="form-group">
					<label for="email">Email</label> <input name="email" id="email"
						class="form-control" value="${olduser.email}" required="true" />
				</div>
				<div class="form-group">
					<label for="batch">Batch</label> <select name="batch" id="batch"
						class="form-control" required="true">
						<!-- <option value="${olduser.batch}" selected>${olduser.batch}</option> -->
				...
				<div class="form-group">
					<label for="session">Session</label> <input name="session"
						id="session" class="form-control" value="${olduser.session}"
						required="true" />
				</div>
				<div class="form-group">
					<label for="address">Address</label> <input name="address"
						id="address" class="form-control" value="${olduser.address}"
						required="true" />
				</div>
				<div class="form-group">
					<label for="contact">Phone</label> <input name="contact"
						id="contact" class="form-control" value="${olduser.contact}"
						required="true" />
